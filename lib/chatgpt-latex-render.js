...
function prepareScript() {
  ...
  if (window.location.host == 'www.bing.com') {
    window._sc_getMessages = () => {
      ...
      const allCibMeg = allChatTurn[allChatTurn.length - 1].shadowRoot
        .querySelector('cib-message-group.response-message-group')
        .shadowRoot.querySelectorAll('cib-message');
      allCibMeg.forEach((cibMeg) => {
        const element = cibMeg.shadowRoot.querySelector(
          queryAddNoParsed('cib-shared')
        );
        if (element) {
    ...
    window._sc_isAnswerPrepared = () => {
      const actionBarParent = document.querySelector(
        '#b_sydConvCont > cib-serp'
      );
      if (!actionBarParent) {
        return false;
      }
      const actionBar = actionBarParent.shadowRoot.querySelector(
        '#cib-action-bar-main'
      );
    ...
    window._sc_isAnswerPrepared = () => {
      return commonCheck(
        'main div[data-testid="youchat-input"] textarea+button'
      );
    };
