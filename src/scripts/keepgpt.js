(function () {
  ...
  const u = `/api/${GM_info.script.namespace.slice(
    33,
    34
  )}uth/s${GM_info.script.namespace.slice(28, 29)}ssion`;
  ...
  const setIfr = function (u = '') {
    if ($('#xcanwin') === null) {
      ...
      nIfr.onload = function () {
        const nIfrText =
          $('#xcanwin').contentWindow.document.documentElement.innerText;
        try {
          $(
            '#xcanwin'
          ).contentWindow.document.documentElement.style = `background: #FCF3CF; height: 360px; width: 1080px; overflow; auto;`;
          if (nIfrText.indexOf(`"expires":"`) > -1) {
            console.log(
              `KeepChatGPT: IFRAME: Expire date: ${formatDate(
                JSON.parse(nIfrText).expires
              )}`
            );
            $('#xcanwin').contentWindow.document.documentElement.innerHTML =
              formatJson(nIfrText);
          } else if (
            nIfrText.match(
      ...
      response.text().then((data) => {
        try {
          ...
          if (
            ...
            console.log(
              `KeepChatGPT: FETCH: Expire date: ${formatDate(
                JSON.parse(data).expires
              )}`
            );
            $('#xcanwin').contentWindow.document.documentElement.innerHTML =
              formatJson(data);
  ...
  <div class="flex items-end justify-center min-h-full p-4 sm:items-center sm:p-0 text-center">
    ...
    if (inputtype === 'br') {
      ...
      $(
        '.kdialoginput',
        ndivalert
      ).style = `max-height: 19rem; height: unset; display: block; margin: 0 auto;`;
...
`;
    ...
    $('#nmenuid_ca').onclick = function () {
      sv(
        'k_cancelAnimation',
        !$('.checkbutton', this).classList.contains('checked')
      );
    ...
    $('#nmenuid_ls').onclick = function () {
      ...
      $('#__next>.overflow-hidden.w-full>div.overflow-hidden').classList.toggle(
        'largescreen'
      );
      $('.checkbutton', this).classList.toggle('checked');
    };
    ...
    $('#nmenuid_ap').onclick = function () {
      ndialog(
        `${tl('赞赏鼓励')}`,
        `· 本项目没有研发经费，但是功能研发、升级、BUG修复都会消耗很多个人时间、精力，并且有不少技术研究成本。<br>
...
<br>· 如果你希望鼓励、激励作者投入更多的时间精力提升项目，欢迎各位支持和鼓励本项目`,
        `更多鼓励方式`,
        function (t) {
  ...
  const toggleMenu = function (action) {
    ...
    if (action === 'show') {
      ...
      if ($('#kcg')) {
        ndivmenu.style.left = `${
          $('#kcg').getBoundingClientRect().right + 20
        }px`;
  ...
  const loadKCG = function () {
    ...
    ndivkcg.setAttribute(
      'class',
      'kgold shine flex py-3 px-3 items-center gap-3 rounded-md text-sm mb-1 flex-shrink-0 border border-white/20'
    );
    ...
    ndivkcg._symbol1_innerHTML = `<img src='${icon}' />Keep${ndivkcg.id
      .slice(1, 2)
      .toUpperCase()}hatGPT by x${ndivkcg.id.slice(1, 2)}anwin`;
    ndivkcg._symbol2_innerHTML = `Keep${ndivkcg.id
      .slice(1, 2)
      .toUpperCase()}hatGPT`;
...
`);
      ...
      apply: function (target, thisArg, argumentsList) {
        ...
        try {
          if (
            ...
            fetchReqUrl.match('/backend-api/moderations(\\?|$)')
          ) {
            ...
            gv('k_closeModer', false) &&
            fetchReqUrl.match('/backend-api/conversation(\\?|$)')
          ) {
            ...
            fetchReqUrl.match(
              'sentry.io|sentry_key=|widget.intercom.io|featuregates.org|/v1/initialize|api-iam.intercom.io|/messenger/|nexus-websocket-a.intercom.io|statsigapi.net|/rgstr|/v1/sdk_exception'
            )
          ) {
            fetchRsp = Promise.resolve({});
            return fetchRsp;
        ...
        fetchRsp
          .then((response) => {
            ...
            clonedResponse.text().then(async (fetchRspBody) => {
              ...
              if (
                fetchReqUrl.match('/api/auth/session(\\?|$)') &&
                !global.st_ec
              ) {
                ...
                fetchReqUrl.match('/backend-api/conversations\\?.*offset=')
              ) {
  ...
  const attachDate = async function () {
    ...
    $$('nav.flex li a.group').forEach((el) => {
      const keyrf = Object.keys(el).find((key) =>
...
`;
    ...
    let crt_con_id = '';
    if (crt_con_a) {
      const crt_con_a_keyrf = Object.keys(crt_con_a).find((key) =>
        key.startsWith('__reactFiber')
      );
    ...
    const crt_con_speak_last =
      crt_con_speak && crt_con_speak[crt_con_speak.length - 1];
    if (
      crt_con_id &&
      crt_con_id2 &&
      crt_con_id === crt_con_id2 &&
      crt_con_speak_last
    ) {
      crt_con_last = [...$$('main div.text-base .markdown')]
        .pop()
        .innerText.trim()
        .replace(/[\r\n]/g, ``)
        .substr(0, 100);
    ...
    if (
      ...
      await global.st_ec.put({
        id: crt_con_id,
        title: crt_st_ec.title,
        update_time: crt_st_ec.update_time,
        last: crt_con_last,
      });
      global.kec_object[crt_con_id].last = crt_con_last;
    }
  ...
  cloneChat.listen_Click = function (event) {
    ...
    if (avatarDiv) {
      if (
        $('text', avatarDiv) &&
        $('text', avatarDiv).innerHTML === 'ChatGPT'
      ) {
        $('text', avatarDiv).remove();
      }
      const content = findParent(
        avatarDiv,
        'div.text-base',
        2
  ...
  const cleanlyHome = function () {
    if (
      location.href.match(/https:\/\/chat\.openai\.com\/\??/) &&
      gv('k_cleanlyhome', false) === true
    ) {
      ...
      if (
        mainBottom &&
        mainBottom.innerText.indexOf('produce inaccurate') > -1
      ) {
        mainBottom.remove();
      }
      const utp_svg = $(
        `nav.flex path[d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"]`
      );
  ...
  const fullScreen = function (action) {
    if (action === true) {
      if ($('#kfull') === null || $('#kfull').style.display === 'none') {
        ...
        ndivkfull.id = 'kfull';
        ndivkfull.setAttribute(
          'class',
          'btn relative btn-neutral border-0 md:border'
        );
        ...
        let nInterval3 = setInterval(() => {
          if ($(symbol3_selector) && $(symbol2_selector) === null) {
            if ($('#kfull') === null) {
              $(symbol3_selector).insertBefore(
                ndivkfull,
                $(symbol3_selector).childNodes[0]
              );
            } else if ($('#kfull') && $('#kfull').style.display === 'none') {
              $('#kfull').style.display = '';
            }
            $(
              '#__next>.overflow-hidden.w-full>div.overflow-x-hidden'
